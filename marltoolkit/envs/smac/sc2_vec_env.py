...
class SubprocVecEnvSC2(VecEnv):
    ...
    def __init__(self, env_fns, context='spawn'):
        ...
        self.dim_obs, self.dim_state, self.dim_act = self.dim_obs, env_info[
            'state_shape'], self.dim_act
    ...
    def step_async(self, actions):
        ...
        for env_done, remote, action in zip(self.buf_done, self.remotes,
                                            actions):
            if not env_done:
    ...
    def step_wait(self):
            ...
            for idx_env, env_done, remote in zip(
                ...
                if not env_done:
                    ...
                    self.buf_obs[idx_env], self.buf_state[idx_env] = np.array(
                        obs), np.array(state)
                    self.buf_rew[idx_env], self.buf_terminal[
                        idx_env] = np.array(rew), np.array(terminal)
        ...
        ), self.buf_state.copy(), self.buf_rew.copy(), self.buf_terminal.copy(
