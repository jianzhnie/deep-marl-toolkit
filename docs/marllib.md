...
[![../_images/marl_env_right.png](https://marllib.readthedocs.io/en/latest/_images/marl_env_right.png)](https://marllib.readthedocs.io/en/latest/_images/marl_env_right.png)
...
[![../_images/rllib_data_flow_left.png](https://marllib.readthedocs.io/en/latest/_images/rllib_data_flow_left.png)](https://marllib.readthedocs.io/en/latest/_images/rllib_data_flow_left.png)
...
在 MARLlib 中，由于 RLlib 提供了许多算法，实现独立学习（左）非常简单。要开始训练，可以从 RLlib 中选择一种算法并将其应用到多智能体环境中，与 RLlib 相比，无需额外的工作。尽管 MARL 中的独立学习不需要任何数据交换，但在大多数任务中其性能通常不如Centralized 式训练策略。

### Centralized Critic

Centralized 式Critic 是 MARLlib 支持的 CTDE 框架中的两种Centralized 式训练策略之一。在这种方法下，智能体需要在获得策略输出之后但在临界值计算之前相互共享信息。这些共享信息包括 独立观察、动作和全局状态（如果有）。

交换的数据在采样阶段被收集并存储为过渡数据，其中每个过渡数据都包含自收集的数据和交换的数据。然后利用这些数据来优化Centralized 式批评函数和分散式政策函数。信息共享的实现主要是在同策略算法的后处理函数中完成的。对于像 MADDPG 这样的Off-Policy算法，在数据进入训练迭代批次之前会收集其他数据，例如其他智能体提供的动作值。

### 价值分解

在 MARLlib 中，价值分解（VD）是另一类Centralized 式训练策略，与Centralized 式批评者的不同之处在于需要智能体共享信息。具体来说，仅需要在智能体之间共享预测的 Q 值或临界值，并且根据所使用的算法可能需要额外的数据。例如，QMIX 需要全局状态来计算混合 Q 值。
...
MARL 算法采用Centralized 训练和分散执行（CTDE）范式，需要在学习阶段在Agent之间共享信息。在 QMIX、FACMAC 和 VDA2C 等值分解算法中，总 Q 或 V 值的计算需要Agent提供各自的 Q 或 V 值估计。相反，基于中心化批评的算法，如 MADDPG、MAPPO 和 HAPPO，需要Agent共享他们的观察和动作数据，以确定中心化批评值。后处理模块是Agent与同伴交换数据的理想位置。对于Centralized 式批评算法，Agent可以从其他Agent获取附加信息来计算Centralized 式批评值。另一方面，对于值分解算法，智能体必须向其他智能体提供其预测的 Q 或 V 值。此外，后处理模块还负责使用 GAE 或 N 步奖励调整等技术来计算各种学习目标。
...
在中心化的批评家Agent模型中，传统的仅基于Agent自我观察的价值函数被能够适应算法要求的centralized critic所取代。centralized critic 负责处理从其他Agent收到的信息并生成 Centralized 值作为输出。
...
为了保证算法的增量单调性，利用信任域来获得合适的参数更新，就像HATRPO算法中的情况一样。为了在考虑计算效率的同时加速策略和批评者更新过程，HAPPO 算法中采用了近端策略优化技术。
...
例如，考虑 MAgent 的混合模式场景，可以使用以下策略映射来表示：
